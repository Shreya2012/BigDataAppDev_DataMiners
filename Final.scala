...
// LinearRegression
val lir = new LinearRegression().setFeaturesCol("scaledFeatures").setMaxIter(1000).setRegParam(0.01).setElasticNetParam(1.0)
val model = lir.fit(trainingData)
val pred = model.evaluate(testData)

// Evaluation
val evaluator = new RegressionEvaluator().setPredictionCol("prediction").setMetricName("rmse")
val rmse2 = evaluator.evaluate(pred.predictions)
...
val rmse3 = evaluator.evaluate(predictions)
